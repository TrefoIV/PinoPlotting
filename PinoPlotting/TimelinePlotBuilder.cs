...
{
    public class TimelinePlotBuilder : AbstractPlot
    {
        ...
        public TimelinePlotBuilder(bool logX, bool logY, bool drawBoxes = true) : base(logX, logY)
        {
            DrawBoxes = drawBoxes;
        }
        ...
        protected void PlotAllTimelines()
        {
        ...
        public override void SavePlot(FileInfo outFile, string xLabel = "", string yLabel = "")
        {
            ...
            else _plt.Axes.Left.TickGenerator = new NumericAutomatic() { LabelFormatter = NumericLabeling };
        ...
        private static string DayDateLabeling(DateTime date)
        {
            return date.DayOfWeek == DayOfWeek.Monday ? date.ToString("d") : $"{date.DayOfWeek.ToString()[0]}";
        }
        private static string MonthDateLabeling(DateTime date)
        {
            return date.ToString("MMM/yyyy", CultureInfo.InvariantCulture);
        }
        private static string FullDateLabeling(DateTime date)
        {
