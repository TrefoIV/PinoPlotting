...
namespace MyPlotting
{
    public class LinePlotBuilder : AbstractPlot
    {
        ...
        public void AddPlot(IEnumerable<double> data, string label = null, Color? color = null, float size = 5f, LinePattern? linePattern = null, MarkerShape marker = MarkerShape.FilledCircle)
        {
        ...
        public override void SavePlot(FileInfo outFile, string xLabel = "", string yLabel = "")
        {
            ...
            _plt.Legend.Location = Alignment.UpperRight;
            ...
            if (Constants.ImageFormat.EndsWith(".png", StringComparison.InvariantCulture))
                _plt.SavePng(outFile.FullName + Constants.ImageFormat, xSize, 600);
            else if (Constants.ImageFormat.EndsWith(".svg", StringComparison.InvariantCulture))
                _plt.SaveSvg(outFile.FullName + Constants.ImageFormat, xSize, 600);
            else
            {
